...
async function run() {
  ...
  assert.ok(result.catalog.count >= 20);
  assert.equal(result.catalog.looks[0].id, 'autumn_female_20_260910_01');
  assert.ok(result.catalog.looks.some(look => look.id === 'autumn_female_20_260911_10'));

  const urls = result.catalog.looks.map(look => look.url);
  assert.equal(urls.length, result.catalog.count);
  ...
  assert.equal(checks.filter(item => item.ok && item.status === 200 && item.contentType === 'image/webp' && item.contentLength > 0).length, result.catalog.count);
  ...
  assert.equal(applied.appliedLooks, result.catalog.count);
  assert.equal(outfitManager.getLooks('female_20s').length, result.catalog.count);
  assert.equal(outfitManager.getLooks('female_20s').filter(look => look.image.startsWith('https://storage.googleapis.com/')).length, result.catalog.count);
